    ...
    @foreach ($courses as $index => $course)
        ...
        <div class="course-card w-80 h-48 mb-3 cursor-pointer" onclick="location.href='/student/courses/{{ $course->id }}'">
            <div class="card rounded-2xl py-1 bg-{{ $currentColor }} hover:bg-{{ $currentColor }}">
                <h3 class="ml-5 mt-3 text-xl @if ($currentColor == 'violet-500') text-white @else text-gray-900 @endif font-bold">{{ $course->name }}</h3>
                <p class="ml-5 mt-1 text-base @if ($currentColor == 'violet-500') text-slate-300 @else font-normal @endif">{{ $course->instructor->fullname }}</p>
